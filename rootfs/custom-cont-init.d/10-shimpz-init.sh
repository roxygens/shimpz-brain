...
# Brain: Claude Code (subscription login OR ANTHROPIC_API_KEY).
upsert ANTHROPIC_API_KEY       "${ANTHROPIC_API_KEY:-}"
# OPENAI_API_KEY/VOICE_TOOLS_OPENAI_KEY are NOT seeded here (SECURITY_ENGINEERING_PLAN.md item 7) —
# voice (STT/TTS) and imagegen call the separate openai-driver container, which is the ONLY
# place the OpenAI key lives now. Seeding it into the brain's own .env would put it right back within
# reach of the brain's Bash tool (the exact prompt-injection exposure the split removes).
...
SHIMPZ_BRAIN_KEYS="ANTHROPIC_API_KEY TELEGRAM_BOT_TOKEN TELEGRAM_ALLOWED_USERS"
...
for _key in $_present_keys; do
    ...
    grep -v "^${_key}=" "$NH/.env" > "$NH/.env.tmp" && mv "$NH/.env.tmp" "$NH/.env"
...
if command -v crontab >/dev/null 2>&1; then
    if [ -z "$(crontab -u abc -l 2>/dev/null)" ]; then
        printf '%s\n' '# shimpz-brain: keepalive so crond runs and `crontab -e` jobs actually execute' \
...
# Seed Shimpz's persona/guardrails (Claude Code reads CLAUDE.md from the working dir). Refresh
# from the image each boot so guardrail updates ship — but keep a user-edited copy if present
# by only overwriting when it differs from a prior image seed is overkill; simplest: always
# refresh the canonical persona (Juliano edits guardrails in the repo, not in /config).
if [ -f /defaults/shimpz/CLAUDE.md ]; then
    cp /defaults/shimpz/CLAUDE.md /config/workspace/CLAUDE.md
fi
...
if [ ! -f "$MEMDIR/MEMORY.md" ] && [ -f /defaults/shimpz/memory/MEMORY.md ]; then
    cp /defaults/shimpz/memory/MEMORY.md "$MEMDIR/MEMORY.md"
...
if [ -d /defaults/shimpz/memory/playbooks ]; then
...
if [ -d /defaults/shimpz/skills ]; then
...
if [ -f /defaults/shimpz/settings.json ]; then
    mkdir -p /config/.claude
    /opt/venv/bin/python3 <<'PY' || true
import json, os
dst = "/config/.claude/settings.json"
defaults = json.load(open("/defaults/shimpz/settings.json"))
cur = {}
...
if [ "${SHIMPZ_FRONTEND_DESIGN:-1}" != "0" ]; then
